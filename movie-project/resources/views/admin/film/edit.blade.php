...
<div class="box box-primary">
    <div class="box-header with-border">
        <div class="col-md-4">
            <h3 class="box-title">Sửa thông tin phim</h3>
        </div>
    </div>
    <form  method="post" action="{{ route('admin_film_update', ['id' => $film->id]) }}" enctype="multipart/form-data">
        ...
        <div class="box-body">
            <div class="form-group {{ $errors->first('name') ? 'has-error' : ''}}">
                ...
                <div class="input-group">
                    ...
                    <input id="name" type="text" class="form-control{{ $errors->has('name') ? ' is-invalid' : '' }}" name="name" placeholder="Vui lòng nhập vào tên phim" value="{{ $film->name }}" required>
                </div>
            ...
            <div class="form-group {{ $errors->first('tag') ? 'has-error' : ''}}">
                ...
                <div class="input-group">
                    ...
                    <input id="tag" type="text" class="form-control{{ $errors->has('tag') ? ' is-invalid' : '' }}" name="tag" placeholder="Vui lòng nhập vào tag cho phim" value="{{ $film->tag }}" required>
                </div>
            ...
            <div class="form-group">
                ...
                <div class="box-body">
                    <div class="col-md-4 col-md-offset-4">
                        <img class="img-responsive" src="{{ asset('/storage/' . $film->image) }}" alt="" id="image_default">
                        <div align="center">
                ...
                <div class="box-body">
                    <div class="col-md-6 col-md-offset-3">
                        <div class="embed-responsive embed-responsive-16by9">
                            <iframe id="trailer_link_default" class="embed-responsive-item" src="{{ asset('/storage/' . $film->trailer_link) }}" frameborder="0" allowfullscreen="">
                            </iframe>
                        </div>
                        <div align="center">
                            <input type="file" id="trailer_link" name="trailer_link" accept="video/*"/>
                        </div>
                        ...
                        @if (!(empty($typies)))
                            @if (!empty($film->type))
                                @foreach ($typies as $type)
                                    @if (in_array($type->id, $film->typeId))
                        ...
                        @if (!(empty($publishers)))
                            @if (!(empty($film->publisher)))
                                @foreach ($publishers as $publisher)
                                    <option value="{{ $publisher->id }}" {{ $film->publisher->id == $publisher->id ? 'selected' : ''}}>{{ $publisher->name }}</option>
                                @endforeach
                            @else
                                @foreach ($publishers as $publisher)
                        ...
                        @if (!(empty($directors)))
                            @if (!(empty($film->director)))
                                @foreach ($directors as $director)
                                    <option value="{{ $director->id }}" {{ $film->director->id == $director->id ? 'selected' : ''}}>{{ $director->name }}</option>
                                @endforeach
                            @else
                                @foreach ($directors as $director)
                        ...
                        @if (!(empty($actors)))
                            @if (!empty($film->actor))
                                @foreach ($actors as $actor)
                                    @if (in_array($actor->id, $film->actorId))
                        ...
                        @if (!(empty($countries)))
                            @if (!(empty($film->country)))
                                @foreach ($countries as $country)
                                    <option value="{{ $country->id }}" {{ $film->country->id == $country->id ? 'selected' : ''}}>{{ $country->name }}</option>
                                @endforeach
                            @else
                                @foreach ($countries as $country)
            ...
            <div class="form-group {{ $errors->first('released') ? 'has-error' : ''}}">
                ...
                <div class="input-group">
                    ...
                    <input id="released" type='text' class="form-control" name="released" value="{{ $film->released }}" placeholder="Vui lòng chọn năm sản xuất" required/>
                </div>
            ...
            <div class="form-group {{ $errors->first('content') ? 'has-error' : ''}}">
                ...
                <div class="input-group">
                    ...
                    <textarea id="content" type="text" class="form-control{{ $errors->has('content') ? ' is-invalid' : '' }}" name="content"  placeholder="Mô tả nội dụng của phim" value="{{ old('content') }}" rows="5" required>{{ $film->content }}</textarea>
                </div>
            ...
            <div class="form-group {{ $errors->first('other_description') ? 'has-error' : ''}}">
                ...
                <div class="input-group">
                    ...
                    <textarea id="other_description" type="text" class="form-control{{ $errors->has('other_description') ? ' is-invalid' : '' }}" name="other_description"  placeholder="Mô tả khác" value="{{ old('other_description') }}" rows="5">{{ $film->other_description }}</textarea>
                </div>
